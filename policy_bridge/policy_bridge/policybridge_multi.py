...
ZONE_LABEL_RE = re.compile(r"[A-Z]")
...
class PolicyBridgeNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("object_positions_topic", "/fleet/object_world_positions")
        ...
        self.declare_parameter("default_forbid_minutes", 4)

        self.declare_parameter("origin_id", os.getenv("ROBOT_ID", socket.gethostname()))
        self.declare_parameter("forbidden_ttl_sec", 0.0)
        self.declare_parameter("softcost_ttl_sec", 0.0)
        self.declare_parameter("objects_ttl_sec", 1.0)
        self.declare_parameter("publish_objects_bus_json", True)
        self.declare_parameter("object_positions_bus_topic", "/fleet/object_world_positions_json")
        ...
        self._origin_id = str(self.get_parameter("origin_id").value)
        self._forbid_ttl = float(self.get_parameter("forbidden_ttl_sec").value)
        self._soft_ttl = float(self.get_parameter("softcost_ttl_sec").value)
        self._objects_ttl = float(self.get_parameter("objects_ttl_sec").value)
        self._publish_objects_bus_json = bool(self.get_parameter("publish_objects_bus_json").value)
        self._object_bus_topic = str(self.get_parameter("object_positions_bus_topic").value)
        ...
        forbid_qos = QoSProfile(depth=1)
        forbid_qos.reliability = ReliabilityPolicy.RELIABLE
        forbid_qos.durability = DurabilityPolicy.TRANSIENT_LOCAL
        ...
        self.object_bus_pub = self.create_publisher(RosString, self._object_bus_topic, 10)

        soft_qos = QoSProfile(depth=1)
        soft_qos.reliability = ReliabilityPolicy.RELIABLE
        soft_qos.durability = DurabilityPolicy.TRANSIENT_LOCAL
    ...
    def _c(self, txt: str, color: Optional[str]) -> str:
        if not (self._ui_color and self._isatty) or not color:
            return txt
        return self.COLORS.get(color, "") + txt + self.COLORS["reset"]
    ...
    def _handle_state_command(self, text: str) -> bool:
        ...
        if re.search(r"(응급\s*(시작|상황)|emergency\s*on)", s):
            self._env_state["emergency"] = True
            handled = True
        if re.search(r"(응급\s*(종료)|emergency\s*off)", s):
            self._env_state["emergency"] = False
            handled = True
        ...
        if handled:
            self._ui_print(
                ...
                f"battery={self._env_state['battery_pct']}% emergency={self._env_state['emergency']}",
                "yellow"
            )
    ...
    def _active_conditions(self) -> set[str]:
        ...
        if self._env_state.get("emergency"):
            conds.add("emergency")
    ...
    def _run_single_mission(self, text: str):
        ...
        self._ui_print("")
        self._ui_print("==================== 🚀 Mission start ====================", "bold")
        self._ui_print(f"User command: {text}", "blue")
        ...
        if perm_from_nl:
            ...
            self._ui_print(f"🧱 Permanent keepouts updated: {', '.join(self._permanent_exclusions) or '-'}", "yellow")
        ...
        cond_rules = plan_json.get("conditional_rules", []) or []
        if cond_rules:
        ...
        self._ui_print(f"🧭 Waypoint order: {' → '.join(names)}", "cyan")
    ...
    def _build_planner_prompt(self, text: str) -> str:
        ...
        return (
            ...
            "    {\"priority\":<int>, \"state_condition\":\"default|fire_alarm|low_battery|emergency\", \"action\":\"forbid|allow|allow_shortest\"}, ...\n"
            " ]} ... ],\n"
            " \"path_id\": null\n"
            "}\n"
            "Hard rules:\n"
            ...
            "- Use conditional_rules when user says '평소/기본' + '하지만/다만/화재시/응급시/배터리 낮을때' etc.\n"
    ...
    def _canonicalize_plan_obj(self, obj: dict, user_text: str) -> dict:
        ...
        ALLOWED_CONDS = {"default", "fire_alarm", "low_battery", "emergency"}
    ...
    def _publish_forbidden(self, zones: List[str]):
        ...
        zones_csv = ",".join(zones)

        payload = {
            "origin_id": self._origin_id,
            "stamp": self._now_stamp(),
            "ttl_sec": float(self._forbid_ttl),
            "type": "forbidden",
            "zones": zones,
            "zones_csv": zones_csv,
        }
        ...
        self._ui_print(f"🚫 Forbidden(fleet): {zones_csv if zones_csv else '-'}", "yellow")
    ...
    def _publish_softcost(self, rules: List[WeightedRule]):
        ...
        payload = {
            "origin_id": self._origin_id,
            "stamp": self._now_stamp(),
            "ttl_sec": float(self._soft_ttl),
            "type": "softcost",
            "overrides": zone_costs,
            "weights": [r.to_dict() for r in rules],
        }
        ...
        pretty = ", ".join([f"{z}:{c}" for z, c in zone_costs.items()]) if zone_costs else "-"
        self._ui_print(f"🟦 SoftCost(fleet): {pretty}", "magenta")
    ...
    def _publish_objects(self, frame: str, positions_xy: List[Tuple[float, float]]):
        ...
        if self._publish_objects_bus_json:
            payload = {
                "origin_id": self._origin_id,
                "stamp": self._now_stamp(),
                "ttl_sec": float(self._objects_ttl),
                "type": "objects",
                "frame": frame,
                "points": [{"x": float(x), "y": float(y)} for (x, y) in positions_xy],
            }
            j = RosString()
            j.data = json.dumps(payload, ensure_ascii=False)
            self.object_bus_pub.publish(j)
            ...
            try:
                ...
                if low in ("h", "help", "?"):
                    self._ui_print(
                        ...
                        "State commands: 'fire alarm on', 'fire alarm off', 'battery 15%', 'emergency on'\n"
    ...
    def _print_status(self):
        ...
        self._ui_print(
            f"state: fire={self._env_state['fire_alarm']} battery={self._env_state['battery_pct']}% emergency={self._env_state['emergency']}",
            "gray"
        )
        self._print_keepouts_snapshot()
        self._print_objects_snapshot()
