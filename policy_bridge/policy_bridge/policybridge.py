...
class PolicyBridgeNode(Node):
    ...
    def _publish_forbidden(self, zones: List[str]):
        msg = RosString()
        msg.data = ",".join([z for z in zones if z in self._labels])
        self.forbidden_pub.publish(msg)
        human = msg.data if msg.data else "-"
        self._ui_print(f"🚫 Forbidden zones (permanent+timed+conditional): {human}", "yellow")
    ...
    def _publish_softcost(self, rules: List[WeightedRule]):
        ...
        self._ui_print(f"🟦 Zone cost overrides: {pretty}", "magenta")
